...
export class MapperComponent implements OnInit {
	...
	getProject(projectId: number) {
		...
		return new Promise<void>((resolve) => {
			this.mapperService.getProject(projectId).subscribe((project: Project) => {
				console.log(project);
				this.project = project;
				const units: Array<Unit> = project.units;
				...
				this.convertUrlToBase64(project.siteplanPath)
	...
	customFilter() {
		const _filter = (data: FilteredValues, filter: string): boolean => {
	...
	save() {
		const units = this.dataSource.data.map(d => ({
			unitId: d.unitId,
			picX: d.picX,
			picY: d.picY
		}));
		...
		this.isSaving = true;
		this.mapperService.save(this.project.projectId, { units }).subscribe(result => {
			console.log(result);
			this.isSaving = false;
		})
	}
