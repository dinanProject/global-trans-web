import { Directive, ElementRef, ContentChild, AfterViewInit, Input } from '@angular/core';
...
export class PanAndZoomDirective implements AfterViewInit {
	...
	zoom(center, factor) {
		...
		const newPosition = {
			x: this.getPannablePosition().x - ((center.x - this.getPannableRect().x) * (factor - 1)),
			y: this.getPannablePosition().y - ((center.y - this.getPannableRect().y) * (factor - 1))
		};
